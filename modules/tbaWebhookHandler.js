...
module.exports = (req, res) => {
  ...
  switch (req.body.message_type) {
    case "match_score":
      var blueTeams = message_data.match.alliances.blue.teams.map((item) =>
        item.replace(/^frc/, "")
      );
      var redTeams = message_data.match.alliances.red.teams.map((item) =>
        item.replace(/^frc/, "")
      );
          ...
          entities.forEach((item) => {
            data.getTeamNumber(item.team_id).then((team) => {
              ...
              if (message_data.match.comp_level != "qm") {
                match_name =
                  compLevelToString(message_data.match.comp_level) +
                  " " +
                  message_data.match.set_number.toString() +
                  " Match " +
                  message_data.match.match_number.toString();
              } else {
                match_name =
                  compLevelToString(message_data.match.comp_level) +
                  " " +
              ...
              if (
                item.type == "all" ||
                (item.type == "team" &&
                  allTeams.some((item) => subscribedTeams.includes(item)))
              ) {
                data.getToken(item.team_id).then((token) => {
                  slack.postMessage(
                    blockMessages.matchJustPlayed(
                      {
      ...
      break;
    case "upcoming_match":
      var allTeams = message_data.team_keys.map((item) =>
        item.replace("frc", "")
      );

      request(
        `https://www.thebluealliance.com/api/v3/match/${message_data.match_key}`,
        {
          headers: {
            ...
            entities.forEach((item) => {
              data.getTeamNumber(item.team_id).then((team) => {
                ...
                if (body.comp_level != "qm") {
                  match_name =
                    compLevelToString(body.comp_level) +
                    " " +
                    body.set_number.toString() +
                    " Match " +
                    body.match_number.toString();
                } else {
                  match_name =
                    compLevelToString(body.comp_level) +
                    " " +
                  ...
                  [
                    {
                      ...
                      text: {
                        type: "mrkdwn",
                        text: `The match schedule for <https://www.thebluealliance.com/event/${
                          message_data.event_key
                        }|*${
                          event.name
                        } ${event.year.toString()}*> has been updated.`,
                      },
                      accessory: {
                        type: "button",
                        text: {
